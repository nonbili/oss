---
title: NouTube
description: NouTube の概要
sidebar:
  order: 1
---
<div class="product-overview-header">
  <img class="product-overview-header__logo" src="/app-logos/noutube.png" alt="NouTube logo" />
  <p class="product-overview-header__meta">Android / デスクトップ</p>
</div>

NouTube は YouTube と YouTube Music 向けの Android / デスクトップアプリです。

モバイル版 YouTube と YouTube Music の Web アプリをベースに、広告ブロック、再生制御、ダウンロード、RSS フィード、ローカルライブラリ管理などの機能を追加します。

## 主な特徴

- 広告なし
- バックグラウンド再生
- 1 つのアプリで動画と音楽に対応
- ログインなしでライブラリや履歴を管理
- 動画ダウンロード
- YouTube チャンネルの RSS フィード
- ライブチャット対応
- カスタム CSS 対応

## 最初にやること

1. YouTube または YouTube Music を開く
2. バックグラウンド再生など、普段使う機能を確認する
3. ライブラリ、履歴、フィード、ダウンロード設定を調整する

## ソースコードと不具合報告

- [nonbili/NouTube](https://github.com/nonbili/NouTube)

ダウンロードは [Install](/ja/noutube/install/) を参照してください。

未翻訳のページでは英語版が表示されます。
